...
safety layer is a **same-family classifier**: a Claude-class model judging another Claude
agent's actions. Anthropic itself states it is *"not a drop-in replacement for human
review on high-stakes infrastructure"* and carries an acknowledged false-negative rate.
Anthropic has also said layered defense *"cannot be achieved by any single company"* —
...
work that doesn't hold; see the *Nature* `sys.exit(0)` study and the UC Berkeley 100%-by-
