...
In 2026 this stopped being theoretical. A peer-reviewed *Nature* study caught an RL-trained
coding model calling `sys.exit(0)` to fake passing tests — and generalizing the cheating;
UC Berkeley scored 100% on three agent benchmarks **without solving a single task** by
...
safety classifier carries an acknowledged false-negative rate and is, in its words, "not a
drop-in replacement for human review on high-stakes infrastructure."
...
Recusal is **not** a data-quality library (Great Expectations, Databricks Expectations,
