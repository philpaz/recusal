...
def wrong_subject(tool_input: dict, active_id: str) -> list:
    ...
    if target != active_id:
        return [Finding.fail("subject_match", severity="CRITICAL",
                             ...
                             target=target, active=active_id)]
...
def destructive_path(tool_input: dict) -> list:
    ...
    if not path.startswith(SAFE_PATH):
        return [Finding.fail("path_allowlist", severity="CRITICAL",
                             message=f"{path} is outside the safe path {SAFE_PATH}", path=path)]
...
def data_exfiltration(tool_input: dict) -> list:
    ...
    if domain not in EMAIL_ALLOWLIST:
        return [Finding.fail("egress_allowlist", severity="CRITICAL",
                             message=f"recipient domain '{domain}' is not on the allowlist", to=to)]
...
def coverage_floor(coverage: float, floor: float = 75) -> list:
    ...
    if coverage < floor:
        return [Finding.fail("coverage_floor", severity="ERROR",
                             message=f"coverage {coverage}% < required {floor}%", coverage=coverage)]
...
def action_budget(count: int, soft: int = 25, hard: int = 100) -> list:
    ...
    if count > hard:
        return [Finding.fail("action_budget", severity="ERROR",
                             message=f"{count} actions exceeds the hard cap {hard}", count=count)]
    if count > soft:
        return [Finding.fail("action_budget", severity="WARNING",
                             message=f"{count} actions over the soft budget {soft}", count=count)]
