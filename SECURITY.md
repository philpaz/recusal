...
| **Obfuscated / evasive shell** (spacing, quoting, escapes, `$IFS`, `base64 -d \| sh`, Windows `rd`/`del`/`Remove-Item`) | The hook normalizes and de-obfuscates the command and matches alternate forms; red-team tested in `tests/test_dogfood_redteam.py`. | A deny-list **cannot** catch a command whose name is built at runtime (hex/char-codes/`eval`/variable indirection). Use an **allowlist** for high-stakes tools. The limit is pinned as a test. |
